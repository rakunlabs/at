...
func TestCreateChatMessage_BumpsSessionUpdatedAt(t *testing.T) {
	...
	// UpdatedAt is surfaced as RFC3339 (second resolution). Sleep just over
	// one second so the post-insert updated_at can be strictly greater.
...
func TestListChatMessages_LimitReturnsMostRecentInChronologicalOrder(t *testing.T) {
	...
	// inserts so created_at strictly orders them even at second precision.
	for i := 0; i < 5; i++ {
		if _, err := store.CreateChatMessage(ctx, service.ChatMessage{
			SessionID: session.ID,
			Role:      "user",
...
// TestListChatMessagesBefore_PagesOlderHistory guards the scroll-up lazy
// loading cursor: given an anchor message ID, the store must return the N
// messages strictly older than the anchor, in chronological order.
func TestListChatMessagesBefore_PagesOlderHistory(t *testing.T) {
	ctx := context.Background()
	store := newTestStore(t, nil)
	...
	// Insert 6 messages. created_at has second precision so several rows
	// may share a timestamp; the ULID id tiebreak keeps ordering stable
	// (a short sleep guarantees distinct ULID millisecond components).
	for i := 0; i < 6; i++ {
		if _, err := store.CreateChatMessage(ctx, service.ChatMessage{
			SessionID: session.ID,
			Role:      "user",
	...
	if len(all) != 6 {
		t.Fatalf("got %d messages, want 6", len(all))
	}

	// Anchor at index 3 → the 2 preceding messages are index 1 and 2.
	page, err := store.ListChatMessagesBefore(ctx, session.ID, all[3].ID, 2)
	if err != nil {
		t.Fatalf("ListChatMessagesBefore: %v", err)
	}
	if len(page) != 2 {
		t.Fatalf("got %d messages, want 2", len(page))
	}
	if page[0].ID != all[1].ID || page[1].ID != all[2].ID {
		t.Errorf("page mismatch: got [%q, %q] want [%q, %q]", page[0].ID, page[1].ID, all[1].ID, all[2].ID)
	}

	// Before the oldest message → empty.
	empty, err := store.ListChatMessagesBefore(ctx, session.ID, all[0].ID, 10)
	if err != nil {
		t.Fatalf("ListChatMessagesBefore oldest: %v", err)
	}
	if len(empty) != 0 {
		t.Errorf("got %d messages before the oldest, want 0", len(empty))
	}

	// Unknown anchor → (nil, nil).
	missing, err := store.ListChatMessagesBefore(ctx, session.ID, "01UNKNOWNANCHORID000000000", 10)
	if err != nil {
		t.Fatalf("ListChatMessagesBefore unknown anchor: %v", err)
	}
	if missing != nil {
		t.Errorf("expected nil for unknown anchor, got %d messages", len(missing))
	}
}
