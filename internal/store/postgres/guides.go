...
func (p *Postgres) ListGuides(ctx context.Context, q *query.Query) (*service.ListResult[service.Guide], error) {
	sql, total, err := p.buildListQuery(ctx, p.tableGuides, q, guideCols...)
	if err != nil {
...
func (p *Postgres) GetGuide(ctx context.Context, id string) (*service.Guide, error) {
	query, _, err := p.goqu.From(p.tableGuides).
		Select(guideCols...).
	...
	var row guideRow
	err = p.db.QueryRowContext(ctx, query).Scan(&row.ID, &row.Title, &row.Description, &row.Icon, &row.Content, &row.CreatedAt, &row.UpdatedAt, &row.CreatedBy, &row.UpdatedBy)
...
func (p *Postgres) CreateGuide(ctx context.Context, g service.Guide) (*service.Guide, error) {
	...
	query, _, err := p.goqu.Insert(p.tableGuides).Rows(
		goqu.Record{
			"id":          id,
			"title":       g.Title,
...
func (p *Postgres) UpdateGuide(ctx context.Context, id string, g service.Guide) (*service.Guide, error) {
	...
	query, _, err := p.goqu.Update(p.tableGuides).Set(
		goqu.Record{
			"title":       g.Title,
			"description": g.Description,
			"icon":        icon,
	...
	return p.GetGuide(ctx, id)
}

func (p *Postgres) DeleteGuide(ctx context.Context, id string) error {
	query, _, err := p.goqu.Delete(p.tableGuides).
		Where(goqu.I("id").Eq(id)).
		ToSQL()
	if err != nil {
