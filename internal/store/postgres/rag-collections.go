...
type ragCollectionRow struct {
	...
	VectorStoreConfig json.RawMessage `db:"vector_store_config"`
...
func (p *Postgres) ListRAGCollections(ctx context.Context, q *query.Query) (*service.ListResult[service.RAGCollection], error) {
	sql, total, err := p.buildListQuery(ctx, p.tableRAGCollections, q, "id", "name", "description", "vector_store_config", "embedding_provider", "embedding_model", "embedding_url", "embedding_api_type", "chunk_size", "chunk_overlap", "created_at", "updated_at", "created_by", "updated_by")
	if err != nil {
...
func (p *Postgres) GetRAGCollection(ctx context.Context, id string) (*service.RAGCollection, error) {
	query, _, err := p.goqu.From(p.tableRAGCollections).
	...
	var row ragCollectionRow
	err = p.db.QueryRowContext(ctx, query).Scan(&row.ID, &row.Name, &row.Description, &row.VectorStoreConfig, &row.EmbeddingProvider, &row.EmbeddingModel, &row.EmbeddingURL, &row.EmbeddingAPIType, &row.ChunkSize, &row.ChunkOverlap, &row.CreatedAt, &row.UpdatedAt, &row.CreatedBy, &row.UpdatedBy)
...
func (p *Postgres) CreateRAGCollection(ctx context.Context, c service.RAGCollection) (*service.RAGCollection, error) {
	...
	query, _, err := p.goqu.Insert(p.tableRAGCollections).Rows(
		goqu.Record{
			"id":                  id,
			"name":                c.Name,
			"description":         c.Description,
			"vector_store_config": vsConfigJSON,
...
func (p *Postgres) UpdateRAGCollection(ctx context.Context, id string, c service.RAGCollection) (*service.RAGCollection, error) {
	...
	return p.GetRAGCollection(ctx, id)
}

func (p *Postgres) DeleteRAGCollection(ctx context.Context, id string) error {
	query, _, err := p.goqu.Delete(p.tableRAGCollections).
		Where(goqu.I("id").Eq(id)).
		ToSQL()
	if err != nil {
...
func ragCollectionRowToRecord(row ragCollectionRow) (*service.RAGCollection, error) {
	...
	if len(row.VectorStoreConfig) > 0 {
		if err := json.Unmarshal(row.VectorStoreConfig, &vsConfig); err != nil {
