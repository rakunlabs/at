...
func (p *Postgres) ListPackSources(ctx context.Context, q *query.Query) (*service.ListResult[service.PackSource], error) {
	sql, total, err := p.buildListQuery(ctx, p.tablePackSources, q, packSourceCols...)
	if err != nil {
...
func (p *Postgres) GetPackSource(ctx context.Context, id string) (*service.PackSource, error) {
	query, _, err := p.goqu.From(p.tablePackSources).
	...
	var row packSourceRow
	err = p.db.QueryRowContext(ctx, query).Scan(&row.ID, &row.Name, &row.URL, &row.Branch, &row.Status, &row.LastSync, &row.Error, &row.CreatedAt, &row.UpdatedAt)
...
func (p *Postgres) CreatePackSource(ctx context.Context, ps service.PackSource) (*service.PackSource, error) {
	...
	query, _, err := p.goqu.Insert(p.tablePackSources).Rows(
		goqu.Record{
			"id":         id,
			"name":       ps.Name,
			"url":        ps.URL,
			"branch":     ps.Branch,
			"status":     ps.Status,
			"last_sync":  toNullString(ps.LastSync),
			"error":      ps.Error,
			"created_at": now,
			"updated_at": now,
		},
...
func (p *Postgres) UpdatePackSource(ctx context.Context, id string, ps service.PackSource) (*service.PackSource, error) {
	...
	query, _, err := p.goqu.Update(p.tablePackSources).Set(
		goqu.Record{
			"name":       ps.Name,
			"url":        ps.URL,
			"branch":     ps.Branch,
	...
	return p.GetPackSource(ctx, id)
}

func (p *Postgres) DeletePackSource(ctx context.Context, id string) error {
	query, _, err := p.goqu.Delete(p.tablePackSources).
		Where(goqu.I("id").Eq(id)).
		ToSQL()
	if err != nil {
