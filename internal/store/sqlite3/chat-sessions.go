...
func (s *SQLite) DeleteChatSession(ctx context.Context, id string) error {
	query, _, err := s.goqu.Delete(s.tableChatSessions).
		Where(goqu.I("id").Eq(id)).
		ToSQL()
	if err != nil {
...
func (s *SQLite) CreateChatMessage(ctx context.Context, msg service.ChatMessage) (*service.ChatMessage, error) {
	...
	query, _, err := s.goqu.Insert(s.tableChatMessages).Rows(
		goqu.Record{
			"id":         id,
	...
	if msg.SessionID != "" {
		if err := s.touchChatSession(ctx, nil, msg.SessionID, now); err != nil {
			return nil, err
		}
	}
...
func (s *SQLite) touchChatSession(ctx context.Context, tx *sql.Tx, sessionID string, now time.Time) error {
	q, _, err := s.goqu.Update(s.tableChatSessions).
		Set(goqu.Record{"updated_at": now.Format(time.RFC3339)}).
...
func (s *SQLite) CreateChatMessages(ctx context.Context, msgs []service.ChatMessage) error {
	...
	for sessionID := range touched {
		if err := s.touchChatSession(ctx, tx, sessionID, now); err != nil {
			return err
		}
	}
...
func (s *SQLite) DeleteChatMessages(ctx context.Context, sessionID string) error {
	query, _, err := s.goqu.Delete(s.tableChatMessages).
		Where(goqu.I("session_id").Eq(sessionID)).
		ToSQL()
	if err != nil {
