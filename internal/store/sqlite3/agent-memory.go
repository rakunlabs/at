...
func (s *SQLite) scanAgentMemoryRow(scanner interface{ Scan(...any) error }) (agentMemoryRow, error) {
...
func (s *SQLite) CreateAgentMemory(ctx context.Context, mem service.AgentMemory) (*service.AgentMemory, error) {
	...
	q, _, err := s.goqu.Insert(s.tableAgentMemory).Rows(
		goqu.Record{
			"id":              id,
			"agent_id":        mem.AgentID,
			"organization_id": mem.OrganizationID,
			"task_id":         mem.TaskID,
			"task_identifier": mem.TaskIdentifier,
...
func (s *SQLite) GetAgentMemory(ctx context.Context, id string) (*service.AgentMemory, error) {
	q, _, err := s.goqu.From(s.tableAgentMemory).
	...
	row, err := s.scanAgentMemoryRow(s.db.QueryRowContext(ctx, q))
...
func (s *SQLite) ListAgentMemories(ctx context.Context, agentID, orgID string) ([]service.AgentMemory, error) {
	...
	for rows.Next() {
		row, err := s.scanAgentMemoryRow(rows)
		if err != nil {
...
func (s *SQLite) SearchAgentMemories(ctx context.Context, agentID, orgID, query string) ([]service.AgentMemory, error) {
	...
	ds := s.goqu.From(s.tableAgentMemory).
		Select(agentMemoryCols...).
		Where(
...
func (s *SQLite) GetAgentMemoryMessages(ctx context.Context, memoryID string) (*service.AgentMemoryMessages, error) {
	q, _, err := s.goqu.From(s.tableAgentMemoryMessages).
	...
	err = s.db.QueryRowContext(ctx, q).Scan(&mid, &messagesJSON)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
	if err != nil {
...
func (s *SQLite) CreateAgentMemoryMessages(ctx context.Context, msgs service.AgentMemoryMessages) error {
	...
	q, _, err := s.goqu.Insert(s.tableAgentMemoryMessages).Rows(
		goqu.Record{
