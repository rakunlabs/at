...
func (s *SQLite) GetMarketplace(ctx context.Context, id string) (*service.Marketplace, error) {
	query, _, err := s.goqu.From(s.tableMarketplaces).
	...
	return s.getMarketplaceByQuery(ctx, query, id)
}

func (s *SQLite) GetMarketplaceByName(ctx context.Context, name string) (*service.Marketplace, error) {
	query, _, err := s.goqu.From(s.tableMarketplaces).
		Select(marketplaceCols...).
	...
	return s.getMarketplaceByQuery(ctx, query, name)
}
...
func (s *SQLite) CreateMarketplace(ctx context.Context, m service.Marketplace) (*service.Marketplace, error) {
	...
	query, _, err := s.goqu.Insert(s.tableMarketplaces).Rows(
		goqu.Record{
			"id":                 id,
			"name":               m.Name,
...
func (s *SQLite) UpdateMarketplace(ctx context.Context, id string, m service.Marketplace) (*service.Marketplace, error) {
	...
	now := time.Now().UTC().Format(time.RFC3339)
	query, _, err := s.goqu.Update(s.tableMarketplaces).Set(
		goqu.Record{
			"name":               m.Name,
			"description":        m.Description,
			"skills":             string(skillsJSON),
	...
	return s.GetMarketplace(ctx, id)
}

func (s *SQLite) DeleteMarketplace(ctx context.Context, id string) error {
	query, _, err := s.goqu.Delete(s.tableMarketplaces).
		Where(goqu.I("id").Eq(id)).
		ToSQL()
	if err != nil {
...
func marketplaceRowToRecord(row marketplaceRow) (*service.Marketplace, error) {
	...
	if row.MCPServers != "" {
		if err := json.Unmarshal([]byte(row.MCPServers), &mcpServers); err != nil {
	...
	if row.DirectMCPServers != "" {
		if err := json.Unmarshal([]byte(row.DirectMCPServers), &directMCPServers); err != nil {
