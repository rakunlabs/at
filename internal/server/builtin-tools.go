...
var builtinTools = []builtinToolDef{
	...
	{Name: "task_create", Description: "Create a persistent task/issue in the AT database. Tasks can be assigned to agents, linked to organizations, and tracked through a full lifecycle (backlog → in_progress → in_review → done).", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"title": map[string]any{"type": "string", "description": "Task title"}, "description": map[string]any{"type": "string", "description": "Task description (markdown supported)"}, "organization_id": map[string]any{"type": "string", "description": "Organization ID to scope this task to"}, "assigned_agent_id": map[string]any{"type": "string", "description": "Agent ID to assign this task to"}, "priority_level": map[string]any{"type": "string", "description": "Priority: critical, high, medium, low", "enum": []string{"critical", "high", "medium", "low"}}, "parent_id": map[string]any{"type": "string", "description": "Parent task ID for sub-tasks"}, "status": map[string]any{"type": "string", "description": "Initial status (default: todo). Options: backlog, todo, in_progress, in_review, done"}}, "required": []string{"title"}}},
