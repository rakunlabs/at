...
// multiIDSchema describes an identifier argument that also accepts a list.
// Declaring the array form is what makes the multi-record call discoverable;
// the executors accept it either way (see multiGetIDs), because models were
// already sending arrays into a single-string read and getting told the
// argument was missing.
func multiIDSchema(description string) map[string]any {
	return map[string]any{
		"anyOf": []any{
			map[string]any{"type": "string"},
			map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "minItems": 1},
		},
		"description": description + " Pass an array to fetch several in one call; the reply is then a results envelope and a failing entry is reported per item instead of failing the call.",
	}
}
...
var builtinTools = []builtinToolDef{
	...
	{Name: "agent_run_cancel", Description: "Cancel an ephemeral background agent_run owned by this execution identity.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"run_id": map[string]any{"type": "string"}}, "required": []string{"run_id"}}},
	...
	{Name: "workflow_get", Description: "Get a workflow's full details including its graph (nodes and edges). Use this to inspect an existing workflow's structure before modifying it.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"id": multiIDSchema("The workflow ID.")}, "required": []string{"id"}}},
	...
	{Name: "workflow_update", Description: "Update an existing workflow. You can update the name, description, and/or the graph. Only provided fields are changed. A new version is created and activated by default so the changes take effect. If activate is false, the update remains a draft and you MUST call workflow_activate with the returned version later.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"id": map[string]any{"type": "string", "description": "The workflow ID to update"}, "name": map[string]any{"type": "string", "description": "New workflow name (optional)"}, "description": map[string]any{"type": "string", "description": "New workflow description (optional)"}, "graph": map[string]any{"type": "object", "description": "New workflow graph with nodes and edges (optional)"}, "activate": map[string]any{"type": "boolean", "description": "Activate the newly created version immediately so changes take effect (default: true)"}}, "required": []string{"id"}}},
	{Name: "workflow_activate", Description: "Activate a workflow version so its changes take effect. Always call this after workflow_update using the version returned by that tool.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"id": map[string]any{"type": "string", "description": "The workflow ID"}, "version": map[string]any{"type": "integer", "description": "The workflow version to activate", "minimum": 1}}, "required": []string{"id", "version"}}},
	...
	{Name: "trigger_get", Description: "Get details of a specific trigger by ID.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"id": multiIDSchema("Trigger ID.")}, "required": []string{"id"}}},
	...
	{Name: "llm_trace_list", Description: "List aggregated LLM traces newest-first. Filter by task, organization, agent, session, source, or status to find problematic agent runs.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"task_id": map[string]any{"type": "string", "description": "Filter by task ID"}, "organization_id": map[string]any{"type": "string", "description": "Filter by organization ID"}, "agent_id": map[string]any{"type": "string", "description": "Filter by agent ID"}, "session_id": map[string]any{"type": "string", "description": "Filter by session/root task ID"}, "source": map[string]any{"type": "string", "description": "Filter by source: agent, chat, workflow, gateway, responses"}, "status": map[string]any{"type": "string", "description": "Filter underlying observations by status: ok or error"}, "limit": map[string]any{"type": "number", "description": "Maximum traces to return (default 20, maximum 100)"}, "offset": map[string]any{"type": "number", "description": "Pagination offset"}}}},
	{Name: "llm_trace_get", Description: "Get observations for one LLM trace in chronological order. Returns compact observation metadata by default; set include_bodies=true for request/response and tool IO previews. Use llm_observation_get for one complete untruncated observation.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"trace_id": map[string]any{"type": "string", "description": "Trace ID"}, "include_bodies": map[string]any{"type": "boolean", "description": "Include clipped request/response bodies and tool input/output previews (default false)"}, "limit": map[string]any{"type": "number", "description": "Maximum observations to return (default 100, maximum 500)"}, "offset": map[string]any{"type": "number", "description": "Pagination offset"}}, "required": []string{"trace_id"}}},
	{Name: "llm_observation_get", Description: "Get one complete LLM observation by ID, including full request/response or tool input/output rehydrated from spill files when available.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"id": multiIDSchema("Observation ID from llm_trace_get.")}, "required": []string{"id"}}},
	...
	{Name: "org_get", Description: "Get an organization's full details including its agent roster (hierarchy).", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"id": multiIDSchema("The organization ID.")}, "required": []string{"id"}}},
	...
	{Name: "provider_get", Description: "Get detailed configuration for a specific LLM provider by key. Shows available models, default model, base URL, and auth type. API keys are redacted.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"key": multiIDSchema("The provider key (e.g. 'openai', 'anthropic', 'groq').")}, "required": []string{"key"}}},
	...
	{Name: "bot_get", Description: "Get a single bot config by ID, including its current custom_commands list, allowed users/agents, and routing settings. Token is redacted.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"id": multiIDSchema("The bot config ID (e.g. '01KQ3AGX7TQY275NBFH8A23751').")}, "required": []string{"id"}}},
	...
	{Name: "connection_get", Description: "Get a connection by ID. By default secrets are redacted (only `*_set` booleans returned). Pass `reveal: true` to receive the actual `client_secret`, `refresh_token`, `api_key`, and `extra` values — use this only when the agent genuinely needs them, since the response is logged in tool history.", InputSchema: map[string]any{
		"type": "object",
		"properties": map[string]any{
			"id":     multiIDSchema("Connection ID."),
	...
	{Name: "node_config_get", Description: "Get a node config by ID, including its full `data` blob with sensitive fields unredacted. Used for editing.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"id": multiIDSchema("The node config ID.")}, "required": []string{"id"}}},
	...
	{Name: "guide_get", Description: "Get a guide by ID, including its full markdown content.", InputSchema: map[string]any{"type": "object", "properties": map[string]any{"id": multiIDSchema("The guide ID.")}, "required": []string{"id"}}},
