<script lang="ts">
  ...
  let bundles = $state<Bundle[]>([]); let presets = $state<Bundle[]>([]); let mappings = $state<Mapping[]>([]); let registry = $state<{key: string; platform_only: boolean}[]>([]);
...
{#if manage}
  <section class="settings-section"><div class="flex flex-wrap justify-between gap-3"><h2 class="settings-section-title">Permission bundles</h2><div class="flex gap-2"><button class="settings-button" disabled={busy} onclick={load}>Reload</button><button class="settings-button" disabled={busy} onclick={() => edit()}>New bundle</button></div></div>
    {#if !bundles.length}<p class="settings-note">No custom bundles. Role presets still supply baseline access.</p>{/if}
    ...
    <details><summary class="font-medium cursor-pointer">Read-only role presets</summary><div class="space-y-4 mt-4">{#each presets as p}<div><strong>{p.name || p.id}</strong><p class="settings-note break-words">{p.keys.join(', ')}</p></div>{/each}</div></details>
...
{/if}
<section class="settings-section"><h2 class="settings-section-title">Effective access explanation</h2>
