...
<div class="settings-page settings-form"><header><h1 class="settings-title">System</h1><p class="settings-subtitle">Installation information and encryption management.</p></header>
  <dl class="grid sm:grid-cols-2 gap-3">{#each [['Application',storeInfo.name],['Version',storeInfo.version],['Commit',storeInfo.commit],['Build date',storeInfo.build_date],['Store',storeInfo.store_type],['Assets root',storeInfo.assets_root]] as [label,value]}<div><dt class="settings-note">{label}</dt><dd class="mt-1 break-all">{value || 'Unavailable'}</dd></div>{/each}</dl>
