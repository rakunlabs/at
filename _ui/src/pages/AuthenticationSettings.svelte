<script lang="ts">
  ...
  let secret = $state(''); let clearSecret = $state(false); let scopes = $state('openid profile email'); let rolesClaims = $state(''); let allowedEmails = $state('');
  ...
  let allowedEmailList = $derived(allowedEmails.split('\n').map(v => v.trim()).filter(Boolean));
  // An allowed-address list needs an address, and the address has to be one the
  // provider vouches for. Without the `email` scope most providers release
  // neither, so every sign-in would be refused for a reason that looks like a
  // typo in the list. Advisory rather than blocking: a provider can be
  // configured to release the claim without the scope being named here.
  let allowlistNeedsEmailScope = $derived(allowedEmailList.length > 0 && !scopes.split(/\s+/).includes('email'));
  async function saveProvider(e: SubmitEvent) { e.preventDefault(); if (!editing || busy || claimSourceMissing) return; const p = editing;
    await run(async () => { const { has_client_secret, ...config } = p; const body = { ...config, scopes: scopes.split(/\s+/).filter(Boolean), roles_claims: rolesClaims.split('\n').map(v => v.trim()).filter(Boolean), allowed_emails: allowedEmailList, ...(secret ? { client_secret: secret } : {}), ...(clearSecret ? { clear_client_secret: true } : {}) }; if (p.id) await identityAPI.put(`identity-providers/${encodeURIComponent(p.id)}`, body); else await identityAPI.post('identity-providers', body); editing = null; await load(); });
  }
...
{#if !isNativeAdmin()}<p class="settings-note">Only installation administrators can configure authentication.</p>{:else}
  ...
  <section class="settings-section"><div class="flex flex-wrap justify-between items-center gap-3"><h2 class="settings-section-title">Identity providers</h2><button class="settings-button" disabled={busy} onclick={() => edit()}>Add provider</button></div>
    ...
    <ul class="settings-list">{#each providers as p}<li class="space-y-2"><div class="flex flex-wrap justify-between gap-3"><div><strong>{p.label}</strong><p class="settings-note">{p.enabled ? 'Enabled' : 'Disabled'} · version {p.version}{p.allowed_emails?.length ? ` · ${p.allowed_emails.length} allowed ${p.allowed_emails.length === 1 ? 'address' : 'addresses'}` : ' · open to everyone at this provider'}</p></div><div class="flex gap-2"><button class="settings-button" disabled={busy} onclick={() => edit(p)}>Edit</button><button class="settings-button" disabled={busy || !p.enabled} onclick={() => run(async () => { await identityAPI.delete(`identity-providers/${encodeURIComponent(p.id)}`, { data: { version: p.version } }); await load(); })}>Disable</button></div></div><p class="settings-note break-all">Callback: {new URL(`auth/external/${encodeURIComponent(p.id)}/callback`, document.baseURI).href}</p></li>{/each}</ul>
    {#if editing}<form class="settings-section" onsubmit={saveProvider}>
      ...
      <label>Allowed email addresses (one per line)<textarea bind:value={allowedEmails} rows={4} spellcheck="false" placeholder="ada@example.com&#10;@firma.com"></textarea><span class="settings-note">Leave empty to let everyone this provider authenticates sign in — the default, and the right setting for a provider that already holds only your own people. Write <code>ada@example.com</code> to allow one address, or <code>@firma.com</code> to allow a whole domain. Case is ignored; subdomains are not included, so <code>@firma.com</code> does not cover <code>@mail.firma.com</code>. Up to 256 entries.</span></label>
      <p class="settings-note">The list is checked at <strong>every</strong> sign-in, not only when an account is first created, so removing an entry takes access away from an account that already exists. It decides whether someone may sign in at all and grants nothing beyond that: workspace access still comes from memberships and permission mappings.</p>
      <p class="settings-note">A listed address is admitted only when the provider reports it as verified (<code>email_verified</code>). A provider that reports no verified address — GitHub's user endpoint, for instance — will refuse every sign-in while this list is non-empty. Leave the list empty for such a provider and restrict access with a workspace permission mapping on a group or role claim instead.</p>
      {#if allowlistNeedsEmailScope}<p role="alert" class="settings-error">The scopes above do not request <code>email</code>. Most providers then release no address and every sign-in will be refused.</p>{/if}
